...
class Watermark {
  ...
  constructor(settings) {
    ...
    Watermark.settings = this.settings
    this.watermarkDom = hasObserver ? new MutationObserver(this._domChangeCallback) : null
  }

  init () {
    _loadMark(this.settings, this.watermarkDom)
  }

  remove () {
    /*移除水印*/
    const watermark_element = document.getElementById(this.settings.watermark_id)
...
function _loadMark (settings, watermarkDom) {
  ...
  if (minotor && hasObserver && watermarkDom) {
    watermarkDom.observe(watermark_hook_element, option);
    watermarkDom.observe(document.getElementById(newSettings.watermark_id).shadowRoot, option);
  }
}